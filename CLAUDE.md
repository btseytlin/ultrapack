...
Internal plugin name: `up`. Claude Code slash and skill invocations use the `up:` prefix: `/up:make`, `up:udesign`, `up:reviewer`. Codex command skills use `$make` and `$try`. Pi uses `/make` or `/skill:make`. Process skills are `u`-prefixed (`udesign`, `uplan`, `uexecute`, `uverify`, `ureview`, `udebug`, `udocument`) to dodge collisions.
